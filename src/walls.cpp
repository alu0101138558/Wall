...
void Walls :: generatePosibleWalls (vector<vector<int>> actualWall, int& actualRow) {
  if (actualWall.size() == heightWall_ && areRowsBalanced(actualWall)) {
...
bool Walls :: areRowsBalanced (vector<vector<int>>& actualWall) const {
  if (actualWall.size() >= 2) {
    ...
    int positionRow1, positionRow2 = 1;
    while (positionRow1 != actualWall[actualWall.size() - 1].size() - 1 || positionRow2 != actualWall[actualWall.size() - 2].size() - 1) {
      if (row1 == row2) {
        return false;
      } else if (row1 > row2) {
